...
class ProjectDetails extends CI_Controller {
	...
	public function one($id)
	{
		...
		foreach($allcategories['categories'] as $alcat)
		{
			...
			if($projects[0]->project_type=='CD' )
			{
				...
				$my_array1[$alcat->item_category]['overalltotal'] = $overalltotal; //getmoney_format(round(($overalltotal/100000),2));
			}
			...
			if($projects[0]->project_type=='NT' )
			{
				...
				$my_array1[$alcat->item_category]['overallverified'] = round(($overallverified/100000));//getmoney_format(round(($overallverified/100000),2));
				// $my_array1[$alcat->item_category]['overalltotal'] = round(($overalltotal/100000)); //getmoney_format(round(($overalltotal/100000),2));
				$my_array1[$alcat->item_category]['overalltotal'] = $overalltotal;//round(($overalltotal/100000)); //getmoney_format(round(($overalltotal/100000),2));
			}
	    ...
	    $AmountwiseBreakupChart_verified = array();
        $AmountwiseBreakupChart_nonverified = array();
        foreach($my_array1 as $my_array1_key=>$my_array1_value){          
            ...
            // $AmountwiseBreakupChart_verified[] = array("label"=> $my_array1_key, "y"=> $verified_amount,"customText" => round($my_array1_value['overallverified'],2)." Lac");
            // $AmountwiseBreakupChart_nonverified[] = array("label"=> $my_array1_key, "y"=> ($total_amount - $verified_amount), "customText" => round($calculation,2)." Lac");


			$AmountwiseBreakupChart_verified[] = array("label"=> $my_array1_key, "y"=> $verified_amount,"customText" => round(($my_array1_value['overallverified']/100000),2)." Lac");
            $AmountwiseBreakupChart_nonverified[] = array("label"=> $my_array1_key, "y"=> ($total_amount - $verified_amount), "customText" => round(($calculation/100000),2)." Lac");
        ...
        $data['AmountwiseBreakupChart_verified']=$AmountwiseBreakupChart_verified;
        $data['AmountwiseBreakupChart_nonverified']=$AmountwiseBreakupChart_nonverified;
      ...
      $data['AmountwiseBreakup_DonutChart_dataPoints']=$AmountwiseBreakup_DonutChart_dataPoints;
		...
		foreach($project_details_array as $project_details_array_key=>$project_details_array_value){
			...
			$category_name = '';
			$count_value = 0;
			foreach($project_details_array_value as $key=>$value){				
				$category_name = $key;
				$count_value = count($project_details_array_value[$key]);
			}
		...
		foreach($project_details_array as $project_details_array_key=>$project_details_array_value){
			...
			echo "<pre>verifier_users_array :";
			print_r($verifier_users_array);
			echo "</pre>";
			// exit;
			...
			foreach($verifier_users_array as $verifier_users_array_Key=>$verifier_users_array_value){
				...
				if(isset($project_details_array[$project_details_array_key][$verifier_users_array_value])){
					$ResourcewiseUtilizationChart_dataPoints1[] = array(
						"label"=> $verifier_users_array_value, 
						"y"=> count($project_details_array[$project_details_array_key][$verifier_users_array_value]),
						);
	...
	exit();
		...
		echo "<pre>ResourcewiseUtilizationChart_dataPoints2 :";
		print_r($ResourcewiseUtilizationChart_dataPoints2);
		echo "</pre>";
		// exit;
			...
			$ResourcewiseUtilizationChart_datapoint[] = array(
				...
				"showInLegend"=>true,
				"yValueFormatString"=>"#,##0 ",
				"dataPoints" => $ResourcewiseUtilizationChart_dataPoints2,
			);
	...
	*/
	...
	// 	$data['ResourcewiseUtilizationChart_datapoint'] = $ResourcewiseUtilizationChart_datapoint;
		...
		// echo "<pre>ResourcewiseUtilization_DonutChart_dataPoints :";
		// print_r($data['ResourcewiseUtilization_DonutChart_dataPoints']);
		// echo "</pre>";
		// exit;
