...
class ProjectDetails extends CI_Controller
{
	...
	// Old Funcation RomiWork
	public function one_not_in_use($id)
	{
		...
		foreach ($allcategories['categories'] as $alcat) {
			...
			if ($projects[0]->project_type == 'NT') {
				$my_array1[$alcat->item_category]['percentage'] = ($overalltotal > 0 ? (($overallverified / $overalltotal) * 100) : 0);
				$my_array1[$alcat->item_category]['overallverified'] = ($overallverified / 100000);
				$my_array1[$alcat->item_category]['overalltotal'] = ($overalltotal / 100000);
			}
		}
		...
		foreach ($my_array1 as $my_array1_key => $my_array1_value) {
			...
			// CRITICAL FIX: If total amount is literally 0, CanvasJS Stacked100 throws NaN and vanishes!
			if (empty($total_amount) || $total_amount == 0) {
				continue;
			}

			$calculation = $my_array1_value['overalltotal'] - $my_array1_value['overallverified'];
			$AmountwiseBreakupChart_dataPoints1[] = array("label" => $my_array1_key, "y" => max(0, $verified_amount), "customText" => round($my_array1_value['overallverified'], 2) . " Lac");
			$AmountwiseBreakupChart_dataPoints2[] = array("label" => $my_array1_key, "y" => max(0, $total_amount - $verified_amount), "customText" => round($calculation, 2) . " Lac");
		}
		...
		file_put_contents('C:/Windows/Temp/amountwise.txt', json_encode($AmountwiseBreakupChart_dataPoints1) . "\n\n" . json_encode($AmountwiseBreakupChart_dataPoints2));
	...
	public function three($id)
	{
		...
		$yverifierperc = array();
		if ($listing['ytotal'] > 0 && ($projects[0]->project_type == 'TG' || $projects[0]->project_type == 'CD')) {		//resourcetaggedpieChart	//resourceuntaggedpieChart
		...
		$yverifiernames = array();
		$yverifierperc = array();
		if ($listing['natotal'] > 0 && ($projects[0]->project_type == 'UN' || $projects[0]->project_type == 'CD')) {		//resourceunspecifiedpieChart
			foreach ($listing['projectverifiers'] as $list) {
...
//New Function One Dev: Romi 27-02-2026
...
public function one($id)
        {
                ...
                // Build the new stacked chart data points where X axis (label) is category and stack (name) is verifier
                // First, iterate through the verifier users to create the data series
                foreach ($category_array as $category_array_key => $category_array_value) {
                        $ResourcewiseUtilizationChart_dataPoints1 = array();

                        // Iterate through categories to build the points for each verifier
                        foreach ($verifier_users_array as $verifier_users_array_Key => $verifier_users_array_value) {
                                if (isset($project_details_array[$category_array_value][$verifier_users_array_value])) {
                                        $ResourcewiseUtilizationChart_dataPoints1[] = array("label" => $verifier_users_array_value, "y" => count($project_details_array[$category_array_value][$verifier_users_array_value]));
                                        $ResourcewiseUtilization_DonutChart_dataPoints_array[$category_array_value][] = count($project_details_array[$category_array_value][$verifier_users_array_value]);
                                } else {
                                        $ResourcewiseUtilizationChart_dataPoints1[] = array("label" => $verifier_users_array_value, "y" => 0);
                                }
                        }

                        $ResourcewiseUtilizationChart_datapoint[] = array(
                                "type" => "stackedColumn100",
                                "name" => $category_array_value,
                                "showInLegend" => true,
                                "yValueFormatString" => "#,##0 ",
                                "dataPoints" => $ResourcewiseUtilizationChart_dataPoints1,
                        );
	...
	// New Function DEV :- Romi 27-02-2026
