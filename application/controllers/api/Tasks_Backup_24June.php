...
public function get_project_additionaldata(){
 ...
 public function itemnewall()
    ...
    public function editverified_Backup_22June()      //Backup For 22 June 2026
    {
        $itemid=$this->input->post('item_id');
     ...
     public function editverified_Backup_24June2026()
    {
        ...
        if($edit_opration == 'Update Qty & Details'){
            ...
            $get_instance_details_quantity_verified = $get_instance_details->quantity_verified;
            ...
            if(!empty($get_instance_details_qty_ok)){
                $update_details['qty_ok'] = (int)$get_instance_details_qty_ok - (int)$get_instance_details_quantity_verified;
            }
            if(!empty($get_instance_details_qty_damaged)){
                $update_details['qty_damaged'] = (int)$get_instance_details_qty_damaged - (int)$get_instance_details_quantity_verified;
            }
            if(!empty($get_instance_details_qty_scrapped)){
                $update_details['qty_scrapped'] = (int)$get_instance_details_qty_scrapped - (int)$get_instance_details_quantity_verified;
            }
            if(!empty($get_instance_details_qty_not_in_use)){
                $update_details['qty_not_in_use'] = (int)$get_instance_details_qty_not_in_use - (int)$get_instance_details_quantity_verified;
            }
            if(!empty($get_instance_details_qty_missing)){
                $update_details['qty_missing'] = (int)$get_instance_details_qty_missing - (int)$get_instance_details_quantity_verified;
            }
            if(!empty($get_instance_details_qty_shifted)){
                $update_details['qty_shifted'] = (int)$get_instance_details_qty_shifted - (int)$get_instance_details_quantity_verified;
            }
           
            $update_details['quantity_verified'] = (int)$getquantity[0]->quantity_verified - (int)$get_instance_details_quantity_verified;
            $verify=$this->tasks->update_data($projectname,$update_details,$condition);


            exit;


            echo "<pre>qty_value_of_selected_instance :";
            print_r($qty_value_of_selected_instance);
            echo "</pre>";
            exit;
        ...
        */
        ...
        {
            if($operation == 'addition'){               
                $qty_ok_value = (int)$update_details->quantity_verified;        //22 June 2026 Commented can take code form above
            }else{
                $qty_ok_value = (int)$update_details->quantity_verified;        //22 June 2026 Commented you can take code form above
            }
            $update_details->qty_ok = $qty_ok_value;             
        }
        else if($update_details->item_scrap_condition =='qty_damaged')
        ...
        {
            if($operation == 'addition'){  
                // $quantity_verified = (int)$getquantity[0]->quantity_verified + (int)$update_details->quantity_verified;  //22 June 2026 Commented 
        ...
        unset($new_array[0]["total_item_amount_capitalized"]);
