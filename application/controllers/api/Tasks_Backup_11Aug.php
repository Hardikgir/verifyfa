...
class Tasks extends CI_Controller {
    ...
    public function saveverified()
    {
        ...
        if($scanned->verification_remarks!='')
        {
            ...
            $quantity_verified = (int)$getquantity[0]->quantity_verified + (int)$scanned->quantity_verified;
            // $quantity_verified = (int)$scanned->quantity_verified;
            $scanned->quantity_verified = $quantity_verified;
...
$this->email->attach($file_path);
 ...
 $r = $this->email->send();
    ...
    public function create_issue()
    {
        ...
        $type_of_issue = $_POST['type_of_issue'];
        $company_name = $_POST['company_name'];
        $location = $_POST['location'];
        $project = $_POST['project'];
        $manager_id = $_POST['manager_id'];
        $groupadmin_id = $_POST['groupadmin_id'];
        $issue_title = $_POST['issue_title'];
        $issue_description = $_POST['issue_description'];
        $issue_attachment = '';
        if(!empty($_POST['issue_attachment'])){
        $issue_attachment = $_POST['issue_attachment'];
        }
        
        


        // if(!empty($type_of_issue) && count($type_of_issue) > 0)
        if(!empty($type_of_issue))
		{
            
			header('Content-Type: application/json');
			echo json_encode(array("success"=>200,"message"=>"Issue Created successfully.","data"=>$type_of_issue));
			exit;
		} 
		else {
			header('Content-Type: application/json');
    ...
    public function issue_list()
    {
        ...
        if(!empty($user_id))
		{
			...
			header('Content-Type: application/json');
			echo json_encode(array("success"=>200,"message"=>"Issue Fetch Successfully.","data"=>$user_id));
			exit;
		} 
		else {
			header('Content-Type: application/json');
     ...
     public function view_issue()
    {
        ...
        if(!empty($issue_id) )
		{
			...
			header('Content-Type: application/json');
			echo json_encode(array("success"=>200,"message"=>"Issue Fetch Successfully.","data"=>$issue_id));
			exit;
		} 
		else {
			header('Content-Type: application/json');
