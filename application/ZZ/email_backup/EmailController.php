...
/**
 * Case 4 : Account ID Unlocking Request
 * Proper Working Version
 *
 * Test URL:
 * http://localhost/verifyfa/index.php/EmailController/accountIdUnlocking/5
 */
public function accountIdUnlocking($userId = 1)
{
    ...
    */
    ...
    // Change later from admin table if available
    $receiverName = "Admin User";
    $to           = "solutions@ethicalminds.in";
    ...
    if (empty($to)) {
        echo "Admin Email ID not found.";
        return;
    }

    // Subject
    $subject = "VerifyFA - Request to Unlock Username: " . $userEmail . " under Entity Code: " . $entityCode;
    ...
    */
                ...
                <table width="650" cellpadding="0" cellspacing="0" style="background:#ffffff;border:1px solid #dddddd;padding:30px;">
                    ...
                    <tr>
                        <td style="font-size:14px;color:#333;line-height:26px;padding-bottom:18px;">
                            You are requested to unlock Username:
                            <b>' . $userEmail . '</b>
                            under Entity Code:
                            <b>' . $entityCode . '</b>.
                        </td>
    ...
    /*
    -----------------------------------------
    Send Email Using Working Dynamic Method
    -----------------------------------------
    */
    ...
    if ($result) {
        echo "Unlock request email sent successfully to " . $to;
    } else {
...
//case 4:
     ...
     */
    public function accountIdUnlocking11($userId = 1) {
        $user = $this->Registered_user_model->get_registerd_user($userId);
 ...
 */
public function passwordResetUnlock5($userId = 1)
{
    // Get User Data
    $user = $this->Registered_user_model->get_registerd_user($userId);
...
//     private function _sendEmailDynamic($to,$subject,$message,$attachment='')
// {
//     $this->email->from('solutions@ethicalminds.in','VerifyFA');
//     $this->email->to($to);
